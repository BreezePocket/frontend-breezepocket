...
const MESSAGES: Record<string, string> = {
  ...
  profile: 'Please select an option',
};
...
function renderSuccess(form: HTMLFormElement, firstName: string) {
  ...
  p.textContent = "We've received your request and will reach out to set your team up first.";
  wrap.append(h3, p);
...
function initForm() {
  const form = qs<HTMLFormElement>('#early-access-form');
  if (!form) return;
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const invalidRadio = validateRadios(form, 'profile');
    ...
    try {
      ...
      if (res.status === 404 || res.status === 405) {
        console.warn(`[early-access] ${ENDPOINT} returned ${res.status}: no backend configured; showing success state.`);
      } else if (!res.ok) {
