...
import { type WaitlistState, clearState, emptyState, loadState, saveState } from './state';

/**
 * Multi-step waitlist dialog.
 * Steps: details → assets → profile → wallet → review. Progress is kept in
 * sessionStorage until the form is submitted. Body scroll is locked while open,
...
/** Base58, 32–44 characters: the shape of a Solana public key. */
...
export function initApplyModal(): () => void {
  ...
  const handleInput = qs<HTMLInputElement>('[name="handle"]', modal);
  const walletInput = qs<HTMLInputElement>('[name="wallet"]', modal);
  const interestInput = qs<HTMLTextAreaElement>('[name="interest"]', modal);
  const assetInputs = qsa<HTMLInputElement>('[name="assets"]', modal);
  const experienceInputs = qsa<HTMLInputElement>('[name="experience"]', modal);

  let state: WaitlistState = emptyState();
  let currentStep = 1;
  let isOpen = false;
  let isSuccess = false;
  ...
  const populate = () => {
    ...
    if (interestInput) interestInput.value = state.interest;
    assetInputs.forEach((i) => (i.checked = state.assets.includes(i.value)));
  ...
  const setStepError = (n: number, hasError: boolean) => {
    const slot = stepCard(n)?.querySelector<HTMLElement>('[data-step-error]');
    if (slot) slot.textContent = hasError ? slot.dataset.errorMessage || 'Please make a selection' : '';
  };
  ...
  const validateStep = (n: number): boolean => {
    ...
    if (n === 2) {
      const ok = state.assets.length > 0;
      setStepError(2, !ok);
      if (!ok) assetInputs[0]?.focus();
      return ok;
    }
    if (n === 3) {
    ...
    if (n === 4) {
      // The wallet is optional, but if one is given it should at least look like a Solana address.
      const wallet = state.wallet.trim();
      const ok = wallet === '' || SOLANA_ADDRESS_RE.test(wallet);
      setFieldError(walletInput, !ok);
      if (!ok) walletInput?.focus();
      return ok;
    }
    return true;
  };
  ...
  const renderReview = () => {
    ...
    if (state.handle.trim()) contact.push(`<p>${escapeHtml(state.handle.trim())}</p>`);
    fill('contact', '[data-review-contact]', contact.join(''));

    const assets = assetLabels();
    fill('assets', '[data-review-assets]', assets.length ? `<p>${escapeHtml(assets.join(', '))}</p>` : '');

    const experience = experienceLabel();
    fill('profile', '[data-review-profile]', experience ? `<p>${escapeHtml(experience)}</p>` : '');

    const wallet: string[] = [];
    if (state.wallet.trim()) wallet.push(`<p>${escapeHtml(state.wallet.trim())}</p>`);
    if (state.interest.trim()) wallet.push(`<p>${escapeHtml(state.interest.trim())}</p>`);
    fill('wallet', '[data-review-wallet]', wallet.join(''));
  };
  ...
  const showSuccess = () => {
    ...
    // The header slides away behind the success card.
  ...
  const open = (trigger?: HTMLElement) => {
    ...
    setFieldError(walletInput, false);
  ...
  const submit = async (e: Event) => {
    ...
    try {
      ...
      data.append('handle', state.handle.trim());
      data.append('assets', JSON.stringify(assetLabels()));
      data.append('experience', experienceLabel());
      data.append('wallet', state.wallet.trim());
      data.append('interest', state.interest.trim());

      const res = await fetch(ENDPOINT, { method: 'POST', body: data });
      // No backend in this static build: a missing route answers 404/405 (or 501 on bare static servers).
      if (res.status === 404 || res.status === 405 || res.status === 501) {
        console.warn(`[waitlist] ${ENDPOINT} is not configured in this static build (HTTP ${res.status}); showing the success state anyway.`);
      } else if (!res.ok) {
      ...
      if (submitError) submitError.textContent = submitError.dataset.errorMessage || 'Something went wrong. Please try again.';
    } finally {
  ...
  const onChoiceChange = () => {
    collect();
    if (currentStep === 2 && state.assets.length) setStepError(2, false);
    if (currentStep === 3 && state.experience) setStepError(3, false);
    persist();
  };
  ...
  // `.apply-modal__overlay` has pointer-events: none, so "click outside" is detected on the
  // modal root: anything that is not the card container or the Close button counts as the backdrop.
  ...
  const onEnterInField = (e: KeyboardEvent) => {
    if (e.key === 'Enter' && (currentStep === 1 || currentStep === 4)) {
      e.preventDefault();
      goNext();
    }
  };

  const textInputs = [fullNameInput, emailInput, handleInput, walletInput];
  ...
  interestInput?.addEventListener('input', persist);
  [...assetInputs, ...experienceInputs].forEach((input) => input.addEventListener('change', onChoiceChange));
  ...
  return () => {
    ...
    interestInput?.removeEventListener('input', persist);
    [...assetInputs, ...experienceInputs].forEach((input) => input.removeEventListener('change', onChoiceChange));
